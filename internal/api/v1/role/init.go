...
func SetAPI(router *echo.Group, permissions authHelper.Permissions, dataRepository *common.DataRepository) {
	...
	endpoints := []apiHelper.Endpoint{
		apiHelper.NewEndpoint("/users/:userID/roles", "GET", api.GetRolesByUser, authHelper.Admin),
		apiHelper.NewEndpoint("/users/:userID/roles", "POST", api.GrantRole, authHelper.Admin),
		apiHelper.NewEndpoint("/users/:userID/roles", "DELETE", api.RemoveRole, authHelper.Admin),
	}
